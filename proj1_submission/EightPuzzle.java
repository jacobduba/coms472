...
public class EightPuzzle {
	...
	private static void tryGenerateSuccessor(Move move, State first,
											 OrderedStateList OPEN,
											 OrderedStateList CLOSED) {
		try {
			...
			if (open == null && closed == null) {
				OPEN.addState(suc);
			} else if (open != null && open.cost() > suc.cost()) {
				open.predecessor = first;
				open.numMoves = suc.numMoves;
			} else if (closed != null && closed.cost() > suc.cost()) {
				CLOSED.removeState(closed);
				closed.predecessor = first;
				closed.numMoves = suc.numMoves;
				OPEN.addState(closed);
			}
	...
	public static String AStar(State s0, Heuristic h) {
		...
		while (OPEN.size() != 0) {
			State first = OPEN.remove();
			CLOSED.addState(first);

			if (first.isGoalState()) {
				return solutionPath(first);
			}

			tryGenerateSuccessor(Move.LEFT, first, OPEN, CLOSED);
			tryGenerateSuccessor(Move.RIGHT, first, OPEN, CLOSED);
			tryGenerateSuccessor(Move.UP, first, OPEN, CLOSED);
			tryGenerateSuccessor(Move.DOWN, first, OPEN, CLOSED);
			// TODO DOUBLE MOVES
			if (h == Heuristic.DoubleMoveHeuristic) {
				tryGenerateSuccessor(Move.DBL_LEFT, first, OPEN, CLOSED);
				tryGenerateSuccessor(Move.DBL_RIGHT, first, OPEN, CLOSED);
				tryGenerateSuccessor(Move.DBL_UP, first, OPEN, CLOSED);
				tryGenerateSuccessor(Move.DBL_DOWN, first, OPEN, CLOSED);
			}
		}
	...
	private static String hueToString(Heuristic h) {
		if (h == Heuristic.TileMismatch) return "number of mismatched tiles";
		if (h == Heuristic.ManhattanDist) return "the Manhattan distance";
		return "double moves allowed";
	}

	private static String solutionPathRec(State cur, int moves) {
		if (cur.predecessor == null) {
			return moves + " moves in total (heuristic: " +
					hueToString(State.heu) + ")" + "\n\n" +
					cur.toString() + "\n\n";
		}

		return solutionPathRec(cur.predecessor, moves + 1) +
				cur.move.toString() + "\n\n" + cur.toString() + "\n\n";
	}
